...
def main(argv):
    ...
    if notraj == 0:
        if prog == 'molcas':
            ...
            if not os.path.exists('input'):
                ...
                exit()
        ###new
        elif prog == 'pyrai2mdnequip':
            if not os.path.exists('input'):
                print('\n!!! PyRAI2MD input not found !!!')
                print(usage)
                print('!!! PyRAI2MD input not found !!!')
                exit()
            if not os.path.exists('%s.nequip.pth' % inputs):
                print('\n!!! NequIP compiled model not found !!!')
                print(usage)
                print('!!! NequIP compiled model not found !!!')
                exit()
        ###new
  ...
  ------------------------------------------------
    ...
    ###new
        ...
        gen_pyrai2md(cpus, ensemble, inputs, slpt, sltm, slmm, slnd, slcr, sljb, slin, 'nequip', iformat, shell)
    ###new
...
def pyrai2md(var):
    ...
    ###new
    # copy nequip-compiled model
    if os.path.exists('%s.nequip.pth' % inputs) and not os.path.exists('%s/%s.nequip.pth' % (inputpath, inputs)):
        shutil.copy2('%s.nequip.pth' % inputs, '%s/%s.nequip.pth' % (inputpath, inputname))
    ###new
...
def update_pyrai2md_input(in_temp, inputname):
    ...
    for line in in_temp.splitlines():
        if 'title' in line:
            inputs += 'title %s\n' % inputname
        ###new
        elif 'modeldir'in line:
            inputs += 'modeldir %s.nequip.pth\n' % inputname
        elif 'model_path'in line:
            inputs += 'model_path %s.nequip.pth\n' % inputname
        ###new
        else:
            inputs += '%s\n' % line
